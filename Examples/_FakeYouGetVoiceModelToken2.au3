...
_example()
Func _example()
	Local $aVoices = _FakeYouGetVoicesList(), $aVoicesFromLanguage[], $sModel
	If @error Then
		Switch @error
			Case 1
		...
		Exit
	EndIf
	$aVoicesFromLanguage = _FakeYouGetVoicesFromLanguage($aVoices, "en-us")
	If @error Then
		Switch @error
			Case 1
		...
		Exit
	EndIf
	$sModel = _FakeYouGetVoiceModelToken($aVoices, $aVoicesFromLanguage[0])
	If @error Then
		Switch @error
			Case 1
				MsgBox(16, "Error", "This list is invalid")
